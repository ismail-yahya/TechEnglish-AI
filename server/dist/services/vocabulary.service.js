...
exports.vocabularyService = {
    ...
    async getStats(userId) {
        const { count: learning } = await supabase_1.supabase
            .from("vocabulary_learning")
            .select("*", { count: "exact", head: true })
            .eq("user_id", userId);
        const { count: known } = await supabase_1.supabase
            .from("vocabulary_known")
            .select("*", { count: "exact", head: true })
            .eq("user_id", userId);
    ...
    async addWord(userId, word, translation, level, domain, masteryStatus = "learning") {
        // Check if exists in Known
        const { data: existingKnown } = await supabase_1.supabase
            .from("vocabulary_known")
            .select("id")
            .eq("user_id", userId)
            .eq("word", word)
        ...
        const { data: existingLearning } = await supabase_1.supabase
            .from("vocabulary_learning")
            .select("id")
            .eq("user_id", userId)
            .eq("word", word)
    ...
    async addWordsFromStory(userId, words, level, domain) {
        const results = [];
        for (const w of words) {
            try {
                const res = await this.addWord(userId, w.word, w.translation, level, domain);
    ...
    async getPracticeSession(userId) {
    ...
    async generateExercise(userId, wordId, skill) {
        const { data: word } = await supabase_1.supabase
            .from("vocabulary_learning")
            .select("*")
            .eq("id", wordId)
            .eq("user_id", userId)
        ...
        const { data: user } = await supabase_1.supabase
            ...
            .eq("id", userId)
            .single();
        return gemini_service_1.geminiService.generateExercise(word.word, word.translation, skill, user?.current_level || "A0");
    },
    async recordPracticeResult(userId, wordId, skill, isCorrect) {
        // Determine mapped field
        const mappedSkill = skillFieldMap[skill];
        if (!mappedSkill) {
        ...
        // Update in DB
        await supabase_1.supabase.from("vocabulary_learning").update(updates).eq("id", wordId);
        // Check if mastered (>= 90% or all 4 completed)
        const allCompleted = skills.every((key) => (newState[key] || 0) >= 3);
        if (mastery >= 90 || allCompleted) {
            // Move to Known
            // 1. Insert to Known
            await supabase_1.supabase.from("vocabulary_known").insert({
                user_id: userId,
                word: word.word,
                translation: word.translation,
            });
            // 2. Delete from Learning
            await supabase_1.supabase.from("vocabulary_learning").delete().eq("id", wordId);
            // 3. Increment learned words
            const { data: u } = await supabase_1.supabase
                .from("users")
    ...
    async generateNewWords(userId, count = 10) {
